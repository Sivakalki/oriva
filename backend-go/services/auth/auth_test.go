...
func TestLogin_Success(t *testing.T) {
	svc := NewService(fakeRepo{u: mustUser(t, "pw")}, fakeIssuer{})
...
func TestLogin_UnknownUser(t *testing.T) {
	svc := NewService(fakeRepo{err: errors.New("not found")}, fakeIssuer{})
	_, err := svc.Login(context.Background(), "x@y.com", "pw")
	assertUnauthorized(t, err)
}

func TestLogin_SameErrorForBothFailures(t *testing.T) {
	a := NewService(fakeRepo{u: mustUser(t, "pw")}, fakeIssuer{})
	_, e1 := a.Login(context.Background(), "a@b.com", "wrong")
	b := NewService(fakeRepo{err: errors.New("not found")}, fakeIssuer{})
