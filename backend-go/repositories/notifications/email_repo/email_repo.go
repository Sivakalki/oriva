// Package email_repo sends outbound email (candidate interview invites). The
// "log" transport just logs the rendered message and is the dev default;
// "smtp" sends for real (docs/CLAUDE.md: prefer free/OSS while experimenting).
package email_repo
...
func NewSender(cfg config.Notify, logger *zap.Logger) (Sender, error) {
	...
	case "smtp":
		if cfg.SMTP.Host == "" {
			return nil, fmt.Errorf("email_repo: smtp transport needs notify.smtp.host")
		}
		return &smtpSender{cfg: cfg}, nil
	default:
		return nil, fmt.Errorf("email_repo: unknown transport %q", cfg.Transport)
	}
}
