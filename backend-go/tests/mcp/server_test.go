...
type fakeInterviews struct {
	org     string
	orgErr  error
	plan    *interview_repo.PlanData
	planErr error
}

func (f fakeInterviews) OrgOf(context.Context, string) (string, error) { return f.org, f.orgErr }
func (f fakeInterviews) PlanData(context.Context, string, string) (*interview_repo.PlanData, error) {
...
func connect(t *testing.T, d mcp.Deps) *mcpsdk.ClientSession {
	t.Helper()
	srv := mcp.NewServer(d)
...
func TestListTools(t *testing.T) {
	cs := connect(t, mcp.Deps{Interviews: fakeInterviews{}, Responses: &fakeResponses{}, Sessions: &fakeSessions{}})
...
func TestAdvanceStateTool(t *testing.T) {
	...
	cs := connect(t, mcp.Deps{Interviews: fakeInterviews{org: "o1"}, Responses: &fakeResponses{}, Sessions: fs})

	var out mcp.AdvanceOut
...
func TestRecordTurnTool(t *testing.T) {
	...
	cs := connect(t, mcp.Deps{Interviews: fakeInterviews{org: "o1"}, Responses: fr, Sessions: &fakeSessions{}})

	var out mcp.RecordOut
...
func TestGetInterviewPlanTool(t *testing.T) {
	...
	cs := connect(t, mcp.Deps{Interviews: fakeInterviews{org: "o1", plan: plan}, Responses: &fakeResponses{}, Sessions: &fakeSessions{}})
