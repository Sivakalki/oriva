...
func TestRequireAuth(t *testing.T) {
	...
	t.Run("missing header", func(t *testing.T) {
		...
		middlewares.RequireAuth(j)(okHandler(nil)).ServeHTTP(rr, httptest.NewRequest(http.MethodGet, "/", nil))
	...
	t.Run("bad token", func(t *testing.T) {
		...
		middlewares.RequireAuth(j)(okHandler(nil)).ServeHTTP(rr, req)
		assert.Equal(t, http.StatusUnauthorized, rr.Code)
	})
	...
	t.Run("valid token injects claims", func(t *testing.T) {
		...
		middlewares.RequireAuth(j)(h).ServeHTTP(rr, req)
...
func TestRequireRole(t *testing.T) {
	...
	t.Run("wrong role forbidden", func(t *testing.T) {
		rr := httptest.NewRecorder()
		middlewares.RequireRole(jwt.RoleScheduler)(okHandler(nil)).ServeHTTP(rr, req)
	...
	t.Run("right role passes", func(t *testing.T) {
		rr := httptest.NewRecorder()
		var seen bool
		middlewares.RequireRole(jwt.RoleCandidate)(okHandler(&seen)).ServeHTTP(rr, req)
