...
func TestOrgID_NoClaims(t *testing.T) {
	_, err := handlers.OrgID(httptest.NewRequest(http.MethodGet, "/", nil))
...
func TestJobsCreate(t *testing.T) {
	h := handlers.NewJobsHandler(fakeJobs{})
...
func TestJobsList_Envelope(t *testing.T) {
	body, status, err := handlers.NewJobsHandler(fakeJobs{}).List(httptest.NewRecorder(), authed(http.MethodGet, "/jobs", ""))
	require.NoError(t, err)
...
func TestCandidatesCreate_Conflict(t *testing.T) {
	h := handlers.NewCandidatesHandler(fakeCands{err: apxerrors.E(apxerrors.Conflict, "dup")})
...
func TestInterviewsSchedule(t *testing.T) {
	h := handlers.NewInterviewsHandler(fakeInterviews{}, &fakeSessions{})
...
func TestInterviewsSchedule_NotFound(t *testing.T) {
	h := handlers.NewInterviewsHandler(fakeInterviews{err: apxerrors.E(apxerrors.NotFound, "job not found")}, &fakeSessions{})
...
func TestInterviewsAdvance(t *testing.T) {
	fs := &fakeSessions{}
	h := handlers.NewInterviewsHandler(fakeInterviews{}, fs)
...
func TestInterviewsAdvance_Illegal(t *testing.T) {
	h := handlers.NewInterviewsHandler(fakeInterviews{}, &fakeSessions{err: apxerrors.E(apxerrors.Conflict, "nope")})
...
func TestSessionStatesGraph(t *testing.T) {
	body, status, err := handlers.NewSessionsHandler(fakeGraph{}).Graph(
