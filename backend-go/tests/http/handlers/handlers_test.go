...
func TestHealthHandler(t *testing.T) {
	t.Run("ok", func(t *testing.T) {
		...
		handlers.NewHealthHandler(fakeHealth{true}).Check(rr, httptest.NewRequest(http.MethodGet, "/health", nil))
		assert.Equal(t, http.StatusOK, rr.Code)
	})
	t.Run("down", func(t *testing.T) {
		rr := httptest.NewRecorder()
		handlers.NewHealthHandler(fakeHealth{false}).Check(rr, httptest.NewRequest(http.MethodGet, "/health", nil))
...
func callLogin(t *testing.T, svc fakeAuth, body string) (int, map[string]any) {
	t.Helper()
